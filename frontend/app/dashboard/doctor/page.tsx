        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 flex items-center gap-2">
              <Stethoscope className="text-indigo-600" /> Dr.{" "}
      ...
      <div className="max-w-7xl mx-auto px-6 space-y-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden flex flex-col">
          <div className="p-5 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
            <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
              <Users size={18} className="text-blue-600" /> Active Patients
              Queue
            </h2>
            <Link
            ...
            >
              Full Queue <ChevronRight size={16} />
            </Link>
          </div>
          <div className="p-0">
            <QueueBoard
          ...
          <div className="lg:col-span-2 space-y-3">
            {loading ? (
              ...
              data?.alerts.map((alert: any, idx: number) => (
                <div
                  key={idx}
                  className={`rounded-xl border p-4 flex items-center gap-4 shadow-sm ${alert.type === "urgent" ? "bg-red-50 border-red-200" : "bg-amber-50 border-amber-200"}`}
                >
                  <div
                    className={`p-2 rounded-full ${alert.type === "urgent" ? "bg-red-100 text-red-600" : "bg-amber-100 text-amber-600"}`}
                  >
                    <AlertCircle size={24} />
                  </div>
                  <div className="flex-1">
                    <p
                      className={`font-bold ${alert.type === "urgent" ? "text-red-900" : "text-amber-900"}`}
                    >
                      {alert.message}
                    </p>
                    <p
                      className={`text-sm mt-0.5 ${alert.type === "urgent" ? "text-red-700" : "text-amber-700"}`}
                    >
                      {alert.details}
                    </p>
                  </div>
                  <button
                    className={`px-4 py-2 rounded-lg text-sm font-bold border transition shadow-sm ${alert.type === "urgent" ? "bg-red-600 border-red-700 text-white hover:bg-red-700" : "bg-white border-amber-300 text-amber-800 hover:bg-amber-100"}`}
                  >
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden flex flex-col">
            <div className="p-5 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
              <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                <Calendar size={18} className="text-indigo-600" /> Daily
                Schedule
              </h2>
              <Link
                href="#"
            ...
            <div className="p-5 flex-1 space-y-3">
              {loading ? (
                ...
                data.schedule.map((slot: any, idx: number) => (
                  <div
                    key={idx}
                    className="flex items-center justify-between p-4 bg-white border border-gray-200 rounded-xl hover:border-indigo-300 transition-colors shadow-sm group"
                  >
                    <div className="flex gap-4 items-center">
                      ...
                      <div>
                        <p className="font-bold text-gray-900 text-sm group-hover:text-indigo-700 transition">
                          {slot.patient}
                        </p>
                    ...
                    <div>
                      <span
                        className={`inline-flex items-center px-3 py-1 bg-white border rounded text-[10px] uppercase font-bold tracking-wider ${
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden flex flex-col">
            <div className="p-5 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
              <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                <ClipboardList size={18} className="text-green-600" /> Recent
                Cases
              </h2>
              <Link
                href="/dashboard/diagnosis"
            ...
            <div className="p-5 flex-1 space-y-3">
              {loading ? (
                ...
                data.recentCases.map((case_: any, idx: number) => (
                  <div
                    key={idx}
                    className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 bg-white border border-gray-200 rounded-xl hover:border-green-300 transition-colors shadow-sm"
                  >
                    <div>
