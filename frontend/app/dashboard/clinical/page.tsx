...
export default function ClinicalPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setResponse(result);
      setSuccess("Clinical input processed successfully! Data sent to backend.");
  ...
  return (
    <div className="min-h-screen bg-gray-50 px-4 py-8">
      <div className="mx-auto max-w-4xl">
        <Link href="/dashboard" className="text-indigo-600 hover:text-indigo-700 font-medium inline-flex items-center">
        ...
        <div className="mt-6">
          ...
          <p className="mt-2 text-gray-600">Complete the structured clinical input to generate accurate AI predictions.</p>
        </div>
              ...
              {currentStep === 2 && (
                <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                  <div className="flex items-center gap-2 mb-6">
                    ...
                    <h2 className="text-xl font-semibold text-gray-800">Symptom Assessment</h2>
                  </div>
                  
                  <div className="space-y-2 mb-6">
                    <label className="text-sm font-medium text-gray-700">Duration of Symptoms (days)</label>
                    <input
                      type="number"
                      min="0"
                  ...
                  <div className="space-y-8">
                    {Object.entries(symptomCategories).map(([category, items]) => (
                      <div key={category} className="bg-white rounded-lg border border-gray-100 p-4 shadow-sm">
                        <h3 className="text-md font-semibold text-gray-800 mb-3 border-b pb-2">{category}</h3>
                  ...
                  <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm space-y-8">
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-2">
                        <label className="text-sm font-bold text-gray-700">Pain Level: <span className="text-lg ml-1">{formData.pain_level}/10</span></label>
                      </div>
                      <input
                        type="range"
                        min="0" max="10" step="1"
                      ...
                      />
                      <div className="w-full h-2 rounded-lg mt-2 transition-all duration-300 opacity-70" style={{
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                       <div className="space-y-2">
                        <label className="text-sm font-medium text-gray-700">Swelling Grade</label>
                        <select
                          value={formData.swelling_grade}
                          onChange={(e) => setFormData({ ...formData, swelling_grade: e.target.value })}
                          className="w-full rounded-lg border border-gray-300 px-4 py-2.5 focus:ring-2 focus:ring-indigo-500 outline-none transition"
                        >
                      ...
                      <div className="space-y-2">
                        <label className="text-sm font-medium text-gray-700">Jaw Region</label>
                        <select
                          value={formData.jaw_region}
                          onChange={(e) => setFormData({ ...formData, jaw_region: e.target.value })}
                          className="w-full rounded-lg border border-gray-300 px-4 py-2.5 focus:ring-2 focus:ring-indigo-500 outline-none transition"
                        >
                    ...
                    <div className="space-y-3">
                      <label className="text-sm font-medium text-gray-700 flex justify-between items-center">
                        ...
                        <span className="text-xs bg-indigo-50 text-indigo-700 px-2 py-1 rounded">Selected: {formData.tooth_site}</span>
                      </label>
                      <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                        <div className="text-center font-semibold text-gray-500 text-xs mb-2">MAXILLARY</div>
                        <div className="flex justify-center gap-1 mb-4 flex-wrap">
                          {["18","17","16","15","14","13","12","11"].map(t => (
                            <button key={t} type="button" onClick={() => setFormData({...formData, tooth_site: t})} className={`w-8 h-10 rounded border text-xs font-bold transition-colors ${formData.tooth_site === t ? 'bg-indigo-600 text-white border-indigo-700' : 'bg-white border-gray-300 hover:bg-gray-200 text-gray-700'}`}>{t}</button>
                          ))}
                        ...
                        <div className="text-center font-semibold text-gray-500 text-xs mt-2">MANDIBULAR</div>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {currentStep === 4 && (
                <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                  ...
                  <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
                     <label className="block text-sm font-semibold text-gray-800 mb-4 border-b pb-2">Systemic Conditions</label>
                     <div className="flex flex-wrap gap-2 mb-6">
                       {[
                          ...
                          const isSelected = formData.systemic_conditions.includes(condition);
                          return (
                            <button
                              type="button"
                              key={condition}
                     ...
                     <label className="block text-sm font-semibold text-gray-800 mb-4 border-b pb-2 mt-8">Specific Risk Factors</label>
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {[
                          ...
                          { key: 'bisphosphonate_therapy', label: 'Bisphosphonate Therapy' },
                          { key: 'radiation_therapy_head_neck', label: 'Head & Neck Radiation' },
                        ].map(({ key, label }) => (
                          <label key={key} className={`flex items-center p-3 rounded-lg border cursor-pointer transition-colors ${formData[key as keyof typeof formData] ? 'bg-orange-50 border-orange-200' : 'bg-gray-50 border-gray-200 hover:bg-gray-100'}`}>
                            <input
                              type="checkbox"
                            ...
                            />
                            <span className={`font-medium ${formData[key as keyof typeof formData] ? 'text-orange-900' : 'text-gray-700'}`}>{label}</span>
                          </label>
                        ))}
                     </div>
                  </div>
                  ...
                  {success ? (
                    ...
                    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                      <div className="p-4 bg-gray-50 border-b flex justify-between items-center">
                        <span className="font-semibold text-gray-700">Patient: {formData.patient_id}</span>
                        <span className="text-sm text-gray-500">{formData.age} yrs • {formData.gender}</span>
                      </div>
                      
                      <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-8 text-sm">
                        <div className="space-y-4">
                          <div>
                            <h4 className="text-xs uppercase tracking-wider text-gray-400 font-bold mb-1">Chief Complaint</h4>
                            <p className="text-gray-900 font-medium">{formData.chief_complaint}</p>
                          </div>
                          <div>
                            <h4 className="text-xs uppercase tracking-wider text-gray-400 font-bold mb-1">Symptoms ({formData.symptom_duration_days} days)</h4>
                            <div className="flex flex-wrap gap-1 mt-1">
                              {formData.symptoms.length > 0 ? formData.symptoms.map(s => (
                                <span key={s} className="bg-gray-100 text-gray-700 px-2 py-0.5 rounded text-xs">{s.replace(/_/g, " ")}</span>
                              )) : <span className="text-gray-500 italic">None selected</span>}
                            </div>
                          </div>
                          <div>
                            <h4 className="text-xs uppercase tracking-wider text-gray-400 font-bold mb-1">Pain & Swelling</h4>
                            <p className="text-gray-900">Pain: <span className="font-medium mr-3">{formData.pain_level}/10</span> Swelling: <span className="font-medium">{formData.swelling_grade}</span></p>
                          </div>
                        </div>

                        <div className="space-y-4 md:border-l pl-0 md:pl-8">
                           <div>
                            <h4 className="text-xs uppercase tracking-wider text-gray-400 font-bold mb-1">Clinical Site</h4>
                            <p className="text-gray-900">Tooth: <span className="font-bold text-indigo-700">{formData.tooth_site}</span> ({formData.jaw_region.replace(/_/g, " ")})</p>
                          </div>
                          <div>
                            <h4 className="text-xs uppercase tracking-wider text-gray-400 font-bold mb-1">Medical Highlights</h4>
                            <p className="text-gray-900 mb-1">Smoking: {formData.smoking_status.replace(/_/g, " ")}</p>
                            <div className="flex flex-wrap gap-1 mt-1">
                              {formData.systemic_conditions.filter(c => c !== "None").length > 0 && formData.systemic_conditions.map(c => c !== "None" && (
                                <span key={c} className="bg-red-50 text-red-700 border border-red-100 px-2 py-0.5 rounded text-xs">{c.replace(/_/g, " ")}</span>
                              ))}
                              {formData.bleeding_disorder && <span className="bg-orange-50 text-orange-700 border border-orange-100 px-2 py-0.5 rounded text-xs">Bleeding Disorder</span>}
                              {formData.bisphosphonate_therapy && <span className="bg-orange-50 text-orange-700 border border-orange-100 px-2 py-0.5 rounded text-xs">Bisphosphonates</span>}
                              {formData.radiation_therapy_head_neck && <span className="bg-orange-50 text-orange-700 border border-orange-100 px-2 py-0.5 rounded text-xs">Radiation</span>}
                              {formData.immunocompromised && <span className="bg-orange-50 text-orange-700 border border-orange-100 px-2 py-0.5 rounded text-xs">Immunocompromised</span>}
                              {formData.fever_present && <span className="bg-yellow-50 text-yellow-700 border border-yellow-100 px-2 py-0.5 rounded text-xs">Fever</span>}
                            </div>
                          </div>
                        </div>
                      </div>
                ...
                {currentStep < steps.length ? (
                  ...
                  >
                     {loading ? (
                        ...
                        <>Submit Assessment <CheckCircle size={18} className="ml-2" /></>
                      )}
                  </button>
                )}
              </div>
